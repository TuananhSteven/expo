...
export async function test(t) {
  ...
  t.describe('expo-notifications', () => {
    ...
    if (isInteractive) {
      t.describe('when the app is in background', () => {
        let subscription = null;
        let anyCallbackSpy = null;

        t.beforeEach(async () => {
          anyCallbackSpy = t.jasmine.createSpy('anyHandlerCalled');
          Notifications.setNotificationHandler({
            handleNotification: anyCallbackSpy,
            handleError: anyCallbackSpy,
            handleSuccess: anyCallbackSpy,
          });
          subscription = Notifications.addNotificationReceivedListener(anyCallbackSpy);
        });
        ...
        t.it(
          ...
          async () =>
            await new Promise((resolve, reject) => {
              ...
              let userInteractionTimeout = setTimeout(() => {
                Alert.alert(
              ...
              AppState.addEventListener('change', async state => {
                if (state === 'background' && !notificationSent) {
                  ...
                  t.expect(anyCallbackSpy).not.toHaveBeenCalled();
                  resolve();
                }
              });
            }),
